...
class LockForceDriver(agxSDK.StepEventListener):
    ...
    def pre(self, t):
        ...
        # joint pd control
        force[0]  = 50000*(-4.75 + t*1.5 - pos[0]) - 5000*vel[0]
        ...
        self.setBodyForce(force)
        self.setBodyTorque(torque)
...
def setHeightField(agx_heightField,np_heightField):
...
# Create a simple bucket using the helper functions above
def createBucket(spec):
...
def buildScene1(app, sim, root):
    ...
    agx_heightField = setHeightField(agx_heightField,np_heightField)
    ...
    cuttingEdge, topEdge, forwardVector, bucket = createBucket( default )
...
def buildTheScene(app, sim):
    ...
    ter, shov, driver = buildScene1(app, sim, root)
...
def main(args):
    ...
    # We will loop a number of times
    # and for each loop we will recreate the simulation with
    # different parameters
    for test_run in range(0, 1):

        # Start by cleaning up the simulation from any previous content
        sim.cleanup()

        # Next build a scene with some content
        ter, shov, driver = buildTheScene(app, sim)
        ...
        z = np.zeros(x.shape)
        for hf_index in range(len(x)):
            print(x[hf_index])
            z[hf_index] = hf.getHeight(hf_index,40)
        
        # plt.plot(pos_array[:,0],'r')
        # plt.plot(pos_array[:,1],'g')
        # plt.plot(pos_array[:,2],'b')
        # plt.show()
